...
1. Створення і рендер розмітки по масиву даних `galleryItems` з `app.js` і
   наданим шаблоном.
2. Реалізація делегування на галереї `ul.js-gallery` і отримання `url` великого
   зображення.
3. Відкриття модального вікна при натисканні на елементі галереї.
4. Підміна значення атрибута `src` елемента `img.modal__image`.
5. Закриття модального вікна при натисканні на кнопку
   `button[data-close-modal]`.
6. Очищення значення атрибута `src` елемента `img.modal__image`. Це необхідно
...
- Закриття модального вікна при натисканні на `div.modal__overlay`.
