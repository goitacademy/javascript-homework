...
`calculatePrice(message, pricePerWord)` принимающую строку (в строке будут
...
console.log(
  calculatePrice('Proin sociis natoque et magnis parturient montes mus', 10),
); // 80

console.log(
  calculatePrice('Proin sociis natoque et magnis parturient montes mus', 20),
); // 160
...
Функция принимает 1 параметр `string` - строку, и проверяет ее на содержание
слов `spam` и `sale`. Слова в строке могут быть в произвольном регистре.
